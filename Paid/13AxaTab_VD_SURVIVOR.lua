--==========================================================
--  13AxaTab_VD_SURVIVOR.lua
--  Violence District - Survivor Utilities
--==========================================================
local frame = TAB_FRAME
local tabId = TAB_ID or "vd_survivor"
...
if not VD_Config then
    ...
    msg.Text = "[VD SURVIVOR] Jalankan dulu script Violence District utama agar _G.VD_Config tersedia."
    msg.Parent = frame
    return
end
...
-- Generator
do
    local card = createCard("Generators", "Auto repair generator untuk Survivor.")
    createToggle(card, "Auto Generator", "AUTO_Generator")
    createSlider(card, "Leave Distance", "AUTO_LeaveDist", 10, 30, 2)
    createKeybind(card, "Leave Gen Key", "KEY_LeaveGen")
    createKeybind(card, "Stop Gen Key", "KEY_StopGen")
end

-- Survival
do
    local card = createCard("Survival Tools", "Utility bertahan hidup dari killer.")
    createToggle(card, "No Fall Damage", "SURV_NoFall")
    createToggle(card, "Auto Teleport Flee", "AUTO_TeleAway")
    createSlider(card, "Flee Distance", "AUTO_TeleAwayDist", 20, 80, 5)
    createToggle(card, "Auto Parry", "AUTO_Parry")
    createToggle(card, "Auto Wiggle", "SURV_AutoWiggle")
    createToggle(card, "Perfect Skill Check", "AUTO_SkillCheck")
end

-- Beat Game (Survivor)
do
    local card = createCard("Beat Game (Survivor)", "Auto cari exit map dan finish.")
    createToggle(card, "Beat Survivor", "BEAT_Survivor")
end

------------------------------------------------------------
-- TAB CLEANUP: 13AxaTab_VD_SURVIVOR
------------------------------------------------------------
...
AxaTabCleanup[tabId] = function()
    ...
    C.AUTO_Generator    = false

    -- Survivor survival tools
    C.SURV_NoFall       = false
    C.AUTO_TeleAway     = false
    C.AUTO_Parry        = false
    C.SURV_AutoWiggle   = false
    C.AUTO_SkillCheck   = false

    -- Beat game (survivor)
    C.BEAT_Survivor     = false
end