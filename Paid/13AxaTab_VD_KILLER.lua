--==========================================================
--  13AxaTab_VD_KILLER.lua
--  Violence District - Killer Utilities
--==========================================================
local frame = TAB_FRAME
local tabId = TAB_ID or "vd_killer"
...
if not VD_Config then
    ...
    msg.Text = "[VD KILLER] Jalankan dulu script Violence District utama agar _G.VD_Config tersedia."
    msg.Parent = frame
    return
end
...
-- Combat
do
    local card = createCard("Combat", "Auto attack dan lunge killer.")
    createToggle(card, "Auto Attack", "AUTO_Attack")
    createSlider(card, "Attack Range", "AUTO_AttackRange", 5, 20, 1)
    createToggle(card, "Double Tap (instant hit)", "KILLER_DoubleTap")
    createToggle(card, "Infinite Lunge", "KILLER_InfiniteLunge")
    createToggle(card, "Auto Hook Downed Survivor", "KILLER_AutoHook")
end

-- Hitbox
do
    local card = createCard("Hitbox", "Perbesar hitbox Survivor (killer only).")
    createToggle(card, "Enable Hitbox Expand", "HITBOX_Enabled")
    createSlider(card, "Hitbox Size", "HITBOX_Size", 5, 30, 1)
end

-- Protection
do
    local card = createCard("Protection", "Anti stun, anti blind, dan anti slowdown.")
    createToggle(card, "No Pallet Stun", "KILLER_NoPalletStun")
    createToggle(card, "Anti Blind", "KILLER_AntiBlind")
    createToggle(card, "No Slowdown", "KILLER_NoSlowdown")
end

-- Destruction
do
    local card = createCard("Destruction", "Hancurkan pallet dan generator.")
    createToggle(card, "Full Gen Break", "KILLER_FullGenBreak")
    createToggle(card, "Destroy All Pallets", "KILLER_DestroyPallets")
end

-- Camera
do
    local card = createCard("Camera Helper", "View khusus killer.")
    createToggle(card, "Third Person Camera", "CAM_ThirdPerson")
    createToggle(card, "Force Shift Lock", "CAM_ShiftLock")
end

-- Beat Game
do
    local card = createCard("Beat Game (Killer)", "Auto cari dan habisi semua survivor.")
    createToggle(card, "Beat Killer", "BEAT_Killer")
end

------------------------------------------------------------
-- TAB CLEANUP: 13AxaTab_VD_KILLER
------------------------------------------------------------
...
AxaTabCleanup[tabId] = function()
    ...
    -- Killer auto combat
    C.AUTO_Attack        = false
    C.KILLER_DoubleTap   = false
    C.KILLER_InfiniteLunge = false
    C.KILLER_AutoHook    = false

    -- Hitbox
    C.HITBOX_Enabled     = false

    -- Killer protection
    C.KILLER_NoPalletStun = false
    C.KILLER_AntiBlind    = false
    C.KILLER_NoSlowdown   = false

    -- Killer destruction
    C.KILLER_FullGenBreak   = false
    C.KILLER_DestroyPallets = false

    -- Camera helper (killer focus)
    C.CAM_ThirdPerson    = false
    C.CAM_ShiftLock      = false
